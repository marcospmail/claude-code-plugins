...
def build_block_message(role: str, agents: List[Dict[str, Any]], current_name: Optional[str]) -> str:
    ...
    if is_pm:
        ...
        if team_agents:
            ...
            lines.append("  cd ~/dev/tools/yato && uv run python lib/tmux_utils.py send <target> \"message\"")

            if first_target:
                lines.append("")
                lines.append("Example:")
                lines.append(f"  cd ~/dev/tools/yato && uv run python lib/tmux_utils.py send {first_target} \"Please implement the login feature\"")
        else:
            lines.append("")
        ...
        if pm_agent:
            ...
            lines.append("")
            lines.append("Send a message with:")
            lines.append(f"  cd ~/dev/tools/yato && uv run python lib/tmux_utils.py send {pm_target} \"message\"")
            lines.append("")
            lines.append("Example:")
            lines.append(f"  cd ~/dev/tools/yato && uv run python lib/tmux_utils.py send {pm_target} \"I need help with this task, can you assign another agent?\"")
        else:
            lines.append("")

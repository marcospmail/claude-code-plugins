...
#     cd ${CLAUDE_PLUGIN_ROOT} && uv run yato loop list --status running
...
#     PROJECT_DIR=$(pwd) && cd ${CLAUDE_PLUGIN_ROOT} && uv run yato loop list --status running --project "$PROJECT_DIR"
#
...
# The fix ensures Claude runs: PROJECT_DIR=$(pwd) && cd ${CLAUDE_PLUGIN_ROOT} && uv run yato loop list --project "$PROJECT_DIR"
#

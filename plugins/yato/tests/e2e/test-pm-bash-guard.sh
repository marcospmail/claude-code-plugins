...
# 3. PM is allowed to run HTTP fetches, scripts, source .env files
# 4. PM is blocked from running file-modifying commands (echo, sed, python3, etc.)
# 5. Non-PM agents (developer) are allowed to run any command
# 6. User/orchestrator (no role) is allowed to run any command
# 7. Git read-only subcommands allowed, write subcommands blocked
# 8. Chained commands: blocked if any segment uses a blocked binary
# 9. Invalid JSON stdin returns safe fallback
#
...
echo "======================================================================"
echo "  E2E Test: PM Bash Blocklist Hook"
echo "======================================================================"
echo ""
...
# ============================================================
# Phase 3: PM allowed commands (not on blocklist)
# ============================================================
...
# curl (HTTP fetch - now allowed with blocklist approach)
OUTPUT=$(run_hook_in_pane "0" "curl -s https://example.com")
if echo "$OUTPUT" | grep -q '"continue": true\|"continue":true'; then
    pass "PM allowed: curl"
else
    fail "PM should be allowed to run curl"
fi

# source .env (skill env files - now allowed)
OUTPUT=$(run_hook_in_pane "0" "source ~/.claude/skills/managing-nextcloud-deck/.env")
if echo "$OUTPUT" | grep -q '"continue": true\|"continue":true'; then
    pass "PM allowed: source .env"
else
    fail "PM should be allowed to source .env files"
fi

# bash with arbitrary script (now allowed - not on blocklist)
OUTPUT=$(run_hook_in_pane "0" "bash ~/.claude/skills/managing-nextcloud-deck/scripts/get-board.sh 34")
if echo "$OUTPUT" | grep -q '"continue": true\|"continue":true'; then
    pass "PM allowed: bash with arbitrary script"
else
    fail "PM should be allowed to run bash with any script"
fi

# bash with yato script
...
# jq (data processing)
OUTPUT=$(run_hook_in_pane "0" "jq '.key' file.json")
if echo "$OUTPUT" | grep -q '"continue": true\|"continue":true'; then
    pass "PM allowed: jq"
else
    fail "PM should be allowed to run jq"
fi

# acli (Jira CLI)
OUTPUT=$(run_hook_in_pane "0" "acli jira --action getIssue --issue KEY-123")
if echo "$OUTPUT" | grep -q '"continue": true\|"continue":true'; then
    pass "PM allowed: acli"
else
    fail "PM should be allowed to run acli"
fi
...
# npm (package manager)
OUTPUT=$(run_hook_in_pane "0" "npm install express")
if echo "$OUTPUT" | grep -q '"block"'; then
    pass "PM blocked: npm"
else
    fail "PM should be blocked from running npm"
fi

# vim (editor)
OUTPUT=$(run_hook_in_pane "0" "vim file.txt")
if echo "$OUTPUT" | grep -q '"block"'; then
    pass "PM blocked: vim"
else
    fail "PM should be blocked from running vim"
fi
...
# source && curl chain (both allowed now)
OUTPUT=$(run_hook_in_pane "0" "source ~/.claude/skills/test/.env && curl -s https://example.com | jq .")
if echo "$OUTPUT" | grep -q '"continue": true\|"continue":true'; then
    pass "PM allowed: source && curl | jq chain"
else
    fail "PM should be allowed to chain source && curl | jq"
fi
...
if echo "$BLOCK_OUTPUT" | grep -qi "blocked for PM"; then
    pass "Block reason mentions blocklist"
else
    fail "Block reason should mention blocklist"
fi

...
# Create agents.yml with PM pane_id for new routing
cat > "$TEST_DIR/.workflow/001-test-checkin-suffix/agents.yml" << EOF
pm:
  name: PM
  role: pm
  pane_id: "$PM_PANE_ID"
...
agents: []
EOF
tmux -L "$TMUX_SOCKET" setenv -t "$SESSION_NAME" WORKFLOW_NAME 001-test-checkin-suffix

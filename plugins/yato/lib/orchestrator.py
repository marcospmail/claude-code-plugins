...
class Orchestrator:
    ...
    def start_pm_with_planning_briefing(self, pm_target: str, project_path: str) -> bool:
        ...
        briefing = (
            ...
            f"   TEAM SUGGESTION TEMPLATES: Before proposing, check {yato_path}/templates/team-suggestions/ for YAML templates. "
            f"   Each file (e.g., development.yml, bug.yml) defines a suggested team for a task type. "
            f"   Read the template matching the task type and use it as a starting point for your proposal. "
            f"   Templates are suggestions, not mandatory - adapt the team to the actual task needs. "
            f"   If no template matches, propose a team from scratch. "

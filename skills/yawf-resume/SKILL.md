...
if [[ ! -d "$PROJECT_PATH/.workflow" ]]; then
    ...
    echo "Use /yawf-existing-project or /yawf-new-project to create one."
    exit 1
fi
```
...
<examples>
<example>
<scenario>User runs: /yawf-resume 001-add-user-auth</scenario>
<action>
...
<example>
<scenario>User runs: /yawf-resume (no arguments)</scenario>
<action>
...
<example>
<scenario>User runs: /yawf-resume in a project with no workflows</scenario>
<action>
1. Check .workflow directory doesn't exist
2. Tell user: No workflows found. Use /yawf-existing-project or /yawf-new-project.
</action>
</example>
</examples>
...
**If no workflows:**
```
No workflows found. Use /yawf-existing-project or /yawf-new-project.
```
</output_format>

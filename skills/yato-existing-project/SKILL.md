...
Tell the user: "I'll analyze this project at [PATH]."
...
<examples>
<example>
<scenario>User runs /yato-existing-project "Add hourly cron" from REGULAR TERMINAL (not in tmux)</scenario>
<action>
1. Check tmux → NOT_IN_TMUX
...
<example>
<scenario>User runs /yato-existing-project (no args) from REGULAR TERMINAL (not in tmux)</scenario>
<action>
1. Check tmux → NOT_IN_TMUX
...
<example>
<scenario>User runs /yato-existing-project "Add OAuth" from INSIDE tmux</scenario>
<action>
1. Check tmux → IN_TMUX
2. Get project path: /Users/user/projects/my-app
3. $ARGUMENTS = "Add OAuth" (use directly)
4. Create workflow: init-workflow.sh "Add OAuth" → WORKFLOW_NAME = "001-add-oauth"
5. Targeted analysis using Haiku sub-agent (discovers project type and relevant files)
...
7. Targeted analysis using Haiku sub-agent (discovers project type and relevant files)
